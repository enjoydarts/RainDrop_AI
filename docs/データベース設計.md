...
ユーザー情報、Raindrop OAuth トークン、各種設定を保存します。
...
| anthropic_api_key_encrypted    | TEXT          | Anthropic APIキー（暗号化済み）            |
| openai_api_key_encrypted       | TEXT          | OpenAI APIキー（暗号化済み）               |
| facts_extraction_model         | TEXT          | 事実抽出に使用するモデル（デフォルト: claude-haiku-4-5） |
| summary_generation_model       | TEXT          | 要約生成に使用するモデル（デフォルト: claude-sonnet-4-6） |
| default_summary_tone           | TEXT          | デフォルトトーン（デフォルト: neutral）     |
| notifications_enabled          | INTEGER       | 通知の有効/無効（0: OFF, 1: ON）           |
| default_import_collection_id   | BIGINT        | デフォルト取り込みコレクションID           |
| raindrop_last_imported_at      | TIMESTAMPTZ   | 最終取り込み日時                           |
| monthly_budget_usd             | DECIMAL(10,2) | 月次予算上限（USD）                        |
...
| id            | UUID        | 主キー                                            |
| user_id       | TEXT        | ユーザーID                                        |
| summary_id    | UUID        | 要約ID（NULL可、SET NULL on delete）              |
| raindrop_id   | BIGINT      | 記事ID                                            |
| tone          | TEXT        | トーン                                            |
| status        | TEXT        | ステータス（pending/processing/completed/failed） |
| error         | TEXT        | エラーメッセージ                                  |
| run_count     | INTEGER     | 実行回数（デフォルト: 1）                         |
...
- idx_summary_jobs_user_status
- idx_summary_jobs_user_updated
...
| api_provider  | TEXT          | APIプロバイダー（anthropic/openai/raindrop/extract）    |
| model         | TEXT          | モデル名                                                |
...
| id            | TEXT        | 主キー（UUID）                                          |
| user_id       | TEXT        | ユーザーID                                              |
| type          | TEXT        | 通知種別（import:completed, summary:completed 等）      |
| title         | TEXT        | 通知タイトル                                            |
| description   | TEXT        | 通知詳細                                                |
| data          | JSONB       | 追加情報（JSON）                                        |
| is_read       | INTEGER     | 既読状態（0: 未読, 1: 既読）                            |
| created_at    | TIMESTAMPTZ | 作成日時                                                |
| deleted_at    | TIMESTAMPTZ | 削除日時（論理削除）                                    |
...
週次/月次の読書トレンドをAIが分析・まとめた記録を保存します。

| カラム         | 型          | 説明                                |
| -------------- | ----------- | ----------------------------------- |
| id             | UUID        | 主キー                              |
| user_id        | TEXT        | ユーザーID                          |
| period         | TEXT        | 期間種別（weekly / monthly）        |
| period_start   | TIMESTAMPTZ | 期間開始日                          |
| period_end     | TIMESTAMPTZ | 期間終了日                          |
| content        | TEXT        | LLMが生成したダイジェスト本文      |
| summary_count  | INTEGER     | 対象要約数                          |
| top_themes     | JSONB       | 主要テーマ配列                      |
| created_at     | TIMESTAMPTZ | 作成日時                            |
...
users (1) --< (N) raindrops
users (1) --< (N) summaries
users (1) --< (N) summary_jobs
users (1) --< (N) api_usage
users (1) --< (N) notifications
users (1) --< (N) digests
summaries (1) --< (N) api_usage
summaries (1) --< (N) summary_jobs
```
