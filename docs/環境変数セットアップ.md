...
4. **アプリの設定画面で登録**
   - ログイン後、`/settings` からユーザー単位で登録
   - Anthropic: 要約・ダイジェストに使用
   - OpenAI: 埋め込み・意味検索・テーマ分類に使用
...
**必須度**: 🟡 推奨（レート制限を有効化する場合）

#### ローカル開発環境

Docker Composeに`redis`サービスが含まれているため、追加設定は不要です。
...
- ローカルではDocker Composeの`redis:7-alpine`コンテナを使用
- 本番ではUpstash Redis等のマネージドサービスを推奨
- `ioredis` + `rate-limiter-flexible`で20リクエスト/60秒のレート制限

---

### 5. Ably（リアルタイム通知）

**必須度**: 🟡 推奨（リアルタイム通知機能を使う場合）

#### 取得手順

1. **Ablyにアクセス**
   - https://ably.com/ にアクセス
...
5. **環境変数に設定**
   ```bash
   ABLY_API_KEY=xxxxx.xxxxx:xxxxxxxxxxxxx
   NEXT_PUBLIC_ABLY_KEY=xxxxx.xxxxx:xxxxxxxxxxxxx
   ```

#### 注意事項

- `ABLY_API_KEY`はサーバーサイドで通知送信に使用
- `NEXT_PUBLIC_ABLY_KEY`はクライアントサイドで通知受信に使用
- 無料プランで月間600万メッセージまで利用可能

---

### 6. Inngest Cloud（本番環境用）
...
- [x] `REDIS_URL` - ローカルRedis設定済み
- [ ] `AUTH_RAINDROP_ID` - 要取得
- [ ] `AUTH_RAINDROP_SECRET` - 要取得
- [ ] `ABLY_API_KEY` / `NEXT_PUBLIC_ABLY_KEY` - 要取得（リアルタイム通知を使う場合）
- [ ] Anthropic APIキー - /settings で登録（要約機能を使う場合）
- [ ] OpenAI APIキー - /settings で登録（意味検索・テーマ分類を使う場合）
...
- [ ] `DATABASE_URL` - Supabase / Neon 等
- [ ] `INNGEST_EVENT_KEY` - Inngest Cloud
- [ ] `INNGEST_SIGNING_KEY` - Inngest Cloud
- [ ] `REDIS_URL` - Upstash Redis 等
- [ ] `ABLY_API_KEY` / `NEXT_PUBLIC_ABLY_KEY` - Ably
- [ ] Anthropic APIキー - /settings でユーザー単位に登録
- [ ] OpenAI APIキー - /settings でユーザー単位に登録
