...
import { ToneVariants } from "./tone-variants"
...
export default async function SummaryDetailPage({
  ...
  // 同一記事の全トーン要約を取得
  const allToneSummaries = await withRLS(userId, async (tx) => {
    return await tx
      .select({
        id: summaries.id,
        tone: summaries.tone,
      ...
      .where(
        and(
          eq(summaries.raindropId, summary.raindropId),
  ...
  return (
    <div className="px-4 sm:px-0">
      ...
      <article className="rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm overflow-hidden">
        ...
        <div className="p-8">
          {/* 記事タイトル */}
          <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100 mb-4">{summary.articleTitle}</h1>

          {/* メタ情報 */}
          <div className="flex flex-wrap items-center gap-4 mb-6 pb-6 border-b border-slate-200 dark:border-slate-700">
            <span className="inline-flex items-center gap-1 rounded-full bg-indigo-100 dark:bg-indigo-900/50 px-3 py-1 text-sm font-semibold text-indigo-700 dark:text-indigo-300">
              <ToneIcon className="h-3.5 w-3.5" />
              {toneInfo.label}
            </span>
            {summary.isPublic === 1 && (
              <span className="inline-flex items-center gap-1 rounded-full bg-green-100 dark:bg-green-950/50 px-3 py-1 text-sm font-semibold text-green-700 dark:text-green-300">
          ...
          <div className="mb-6 pb-6 border-b border-slate-200 dark:border-slate-700">
            <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">このトーンで生成</h3>
            <ToneVariants
              raindropId={Number(summary.raindropId)}
              currentSummaryId={summary.id}
              currentTone={summary.tone}
              existingSummaries={allToneSummaries}
            />
          </div>
          ...
          <div className="mb-6 pb-6 border-b border-slate-200 dark:border-slate-700">
            <div className="flex items-center gap-2 mb-2">
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300">テーマ</h3>
            </div>
            <ThemeEditor
          ...
          <div className="prose prose-lg max-w-none">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-slate-700 dark:text-slate-300 m-0">要約</h3>
              <CopyMarkdownButton
                ...
                articleLink={summary.articleLink}
                createdAt={summary.createdAt}
              />
            </div>
            <p className="text-slate-800 dark:text-slate-200 leading-relaxed whitespace-pre-wrap">{summary.summary}</p>
          </div>

          {/* 評価理由 */}
          {summary.ratingReason && (
            <div className="mt-6 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 p-4">
              <h3 className="text-sm font-semibold text-slate-900 dark:text-slate-100 mb-2">評価理由</h3>
              <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed">{summary.ratingReason}</p>
            </div>
          )}
